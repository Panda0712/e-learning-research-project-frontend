...
const Sidebar = () => {
  return (
    <aside className="w-full lg:w-1/4 p-4 bg-white rounded-lg shadow-sm h-fit">
      {/* Search */}
      <div className="mb-8 relative w-full max-w-94.75">
        <input
          type="text"
          placeholder="Search"
      ...
      {/* service category */}
      <div className="mb-8 w-full max-w-94.75">
        ...
        <ul className="flex flex-col gap-3 font-poppins">
          {[
            ...
            <li
              key={index}
              className="
                group w-full h-15 flex items-center justify-between padding px-5 
            ...
            >
              <Link to={`/blog/${id}`} className="shrink-0">
                <img
                  ...
                  alt="Post thumbnail"
                  className="w-24.5 h-24.5 object-cover rounded-md hover:opacity-80 transition-opacity"
                />
              </Link>
