    ...
    <aside className="w-full lg:w-1/4 p-4 bg-white rounded-lg shadow-sm h-fit">
      ...
      <div className="mb-8 relative w-full max-w-[379px]">
        <input 
            type="text" 
            placeholder="Search" 
            className="w-full h-[40px] font-poppins border border-[#E2E1E1] rounded-md px-4 pr-12 text-[16px] focus:outline-none focus:border-blue-500 placeholder-gray-400"
        />
     ...
     {/* service category */}
      <div className="mb-8 w-full max-w-[379px]">
        ...
        <ul className="flex flex-col gap-3 font-poppins">
          {['Soft Skill', 'Soft Skill', 'Soft Skill', 'Soft Skill', 'Soft Skill'].map((cat, index) => (
            <li 
              key={index} 
              className="
      ...
      <div className="mb-8 w-full max-w-[379px]">
        ...
        <div className="flex flex-col">
           {[1, 2, 3, 4].map((id, index) => (
             <div 
                key={index} 
                className={`flex gap-5 py-5 ${index !== 0 ? 'border-t border-gray-200' : ''}`}
             >
                ...
                <div className="flex flex-col justify-center">
                   ...
                   <div className="flex items-center  text-sm font-bold mb-2">
                      ...
                      <span className="font-medium font-poppins text-[#333931] text-[10px]">14 JUNE 2025</span>
                   </div>
      ...
      {/* Popular Tags */}
      <div>
        <h3 className="font-bold text-lg mb-4 border-b-2 border-red-500 inline-block">POPULAR TAGS</h3>
        <div className="flex flex-wrap gap-2">
            {['Soft Skill', 'React', 'Design', 'UI/UX'].map((tag, idx) => (
                <span key={idx} className="bg-gray-100 px-3 py-1 rounded text-sm text-gray-600 hover:bg-blue-100 cursor-pointer">
                    {tag}
                </span>
            ))}
        </div>
