...
const AccountSetting = () => {
  ...
  const getPasswordStrength = () => {
    ...
    if (length === 0) return { label: 'None', color: 'bg-gray-200', text: 'text-gray-400', width: 'w-0' };
    if (length < 6) return { label: 'Weak', color: 'bg-red-500', text: 'text-red-500', width: 'w-1/4' };
    if (length < 10) return { label: 'Medium', color: 'bg-yellow-500', text: 'text-yellow-600', width: 'w-2/4' };
    return { label: 'Strong', color: 'bg-green-500', text: 'text-green-600', width: 'w-3/4' };
  };
            ...
            {activeTab === 'account' && (
                <div>
                    ...
                    <div className="mb-8">
                        ...
                        <textarea name="bio" value={formData.bio} onChange={handleChange} rows={4} className="w-full border border-gray-200 rounded-lg p-3 focus:outline-none focus:border-blue-500 text-sm resize-none"></textarea>
                    </div>

                    <div className="flex gap-4 mb-10">
                        <button className="px-6 py-2.5 rounded-full border border-blue-100 text-blue-600 font-medium hover:bg-blue-50 transition-colors">Cancel</button>
                        <button onClick={handleSaveProfile} className="px-6 py-2.5 rounded-full bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors shadow-md shadow-blue-200">Save & Change</button>
                    </div>
                        ...
                        <div className="space-y-6 max-w-lg">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    <input name="currentPassword" value={passwords.currentPassword} onChange={handlePasswordChange} type={showCurrentPass ? "text" : "password"} className="w-full border border-gray-200 rounded-lg p-3 pr-10 focus:outline-none focus:border-blue-500 text-sm" />
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                     <input name="newPassword" value={passwords.newPassword} onChange={handlePasswordChange} type={showNewPass ? "text" : "password"} className="w-full border border-gray-200 rounded-lg p-3 pr-10 focus:outline-none focus:border-blue-500 text-sm" />
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                     <input name="confirmPassword" value={passwords.confirmPassword} onChange={handlePasswordChange} type={showConfirmPass ? "text" : "password"} className={`w-full border rounded-lg p-3 pr-10 focus:outline-none text-sm ${passwords.confirmPassword && passwords.newPassword !== passwords.confirmPassword ? 'border-red-500 focus:border-red-500' : 'border-gray-200 focus:border-blue-500'}`} />
                                    <button onClick={() => setShowConfirmPass(!showConfirmPass)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                            ...
                            <div>
                                <div className="flex justify-between text-xs text-gray-500 mb-1">
                                    ...
                                    <span className={`font-medium ${passStrength.text}`}>{passStrength.label}</span>
                                </div>
                                <div className="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden transition-all duration-300">
                                    <div className={`h-1.5 rounded-full ${passStrength.color} ${passStrength.width} transition-all duration-500`}></div>
                                </div>
                            </div>

                            <button onClick={handleUpdateSecurity} className="px-6 py-3 rounded-full bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors w-full md:w-auto shadow-md shadow-blue-200">
