...
const AssessmentDetail: React.FC<Props> = ({
  isOpen,
  onClose,
  ...
  useEffect(() => {
    if (isOpen && initialAssessment) {
      const fetchData = async () => {
        setLoading(true);
        setError(null);
        try {
          const [detailedAssessment, submissionsResponse] = await Promise.all([
            assessmentService.getAssessmentByIdAPI(initialAssessment.id),
            assessmentService.getSubmissionsByAssessmentIdAPI(
