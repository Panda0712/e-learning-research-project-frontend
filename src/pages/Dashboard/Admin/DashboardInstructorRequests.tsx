...
import { instructorAdminAPI } from "../../../apis/instructorAdmin";
        ...
        <div className="bg-[#F9FAFB] rounded-xl p-8 border border-gray-100 shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
              ...
              <input
                ...
                readOnly
                className="w-full bg-white border border-gray-200 rounded-lg p-3 text-gray-700 text-sm focus:outline-none"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-500 mb-1">
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                rows={5}
                value={data.bioExperience}
                readOnly
                className="w-full bg-white border border-gray-200 rounded-lg p-3 text-gray-700 text-sm focus:outline-none resize-none"
              />
            </div>
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-gray-500 mb-1">
              ...
              <div className="flex items-center gap-3 bg-white p-3 rounded-lg border border-gray-200">
                ...
                <button
                  onClick={() => window.open(data.cvFileName, "_blank")}
                  className="text-blue-600 text-sm font-bold ml-auto hover:underline flex items-center gap-1"
                >
                  View CV <Eye size={14} />
                </button>
              </div>
          ...
          {data.status === "Pending" && (
            <div className="flex gap-4 mt-8">
              <button
                onClick={onApprove}
                className="px-8 py-3 bg-[#704FE6] text-white rounded-full font-bold hover:bg-[#5F3DD4] transition-all shadow-lg"
              >
                Approve
              </button>
              <button
                onClick={onReject}
                className="px-8 py-3 bg-[#FF4444] text-white rounded-full font-bold hover:bg-[#dd3333] transition-all shadow-lg"
              >
                Reject
              </button>
            </div>
...
const DashboardInstructorRequests = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const res = await instructorAdminAPI.fetchRequests();
      ...
      type RawData = {
        ...
        createdAt: string;
        isActive: boolean;
        lecturerFile?: { fileUrl?: string };
        bio?: string;
      };

      const formattedData: InstructorRequest[] = (dataArray as RawData[]).map((item) => ({
        id: item.id,
        avatar: item.lecturer?.avatar?.fileUrl || "/avatar1.png",
        firstName: item.lecturer?.firstName || "N/A",
        lastName: item.lecturer?.lastName || "N/A",
        email: item.lecturer?.email || "N/A",
        professionalTitle: item.professionalTitle || "N/A",
        appliedDate: new Date(item.createdAt).toLocaleDateString("en-GB"),
        status: item.isActive ? "Approved" : "Pending",
        cvFileName: item.lecturerFile?.fileUrl || "No CV provided",
        bioExperience: item.bio || "No experience provided",
      }));
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100 mb-6">
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#EBEBEB]">
            <tr>
              <th className="p-4 text-sm font-semibold text-gray-600 border-r border-white last:border-r-0">Avatar</th>
              <th className="p-4 text-sm font-semibold text-gray-600 border-r border-white last:border-r-0">Lecturer</th>
              <th className="p-4 text-sm font-semibold text-gray-600 border-r border-white last:border-r-0">Expertise</th>
              <th className="p-4 text-sm font-semibold text-gray-600 border-r border-white last:border-r-0">Applied Date</th>
              <th className="p-4 text-sm font-semibold text-gray-600 text-center border-r border-white last:border-r-0">Status</th>
          ...
          <tbody>
            {loading ? (
              <tr>
                <td colSpan={6} className="p-10 text-center text-gray-500 font-medium italic">
                    ...
                    {openMenuId === item.id && (
                      <div className="absolute right-8 top-10 w-36 bg-white rounded-lg shadow-xl border border-gray-100 z-50 overflow-hidden text-left animate-fade-in">
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 font-medium transition-colors"
                        >
                          <Eye size={16} /> View
                        </button>
                        {activeTab === "requests" && (
                          <>
                            <button
                              ...
                              className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-green-600 hover:bg-green-50 font-bold transition-colors"
                            >
  ...
  function renderConfirmModal() {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-[1px]">
        <div className="bg-white rounded-xl p-8 w-100 shadow-2xl text-center animate-scale-up border border-gray-100">
          <h3 className="text-xl font-bold text-gray-900 mb-3">
            {actionType === "approve" ? "Approve Lecturer?" : "Reject Lecturer?"}
          </h3>
          <p className="text-gray-500 text-sm mb-8 leading-relaxed">
            Are you sure you want to {actionType} this request? This will update the system.
          </p>
          <div className="flex gap-4">
            ...
            <button
              ...
              className="flex-1 py-3 rounded-lg font-bold text-black bg-[#FFD130] hover:bg-[#eec225] shadow-md transition-all"
            >
              Confirm
            </button>
          </div>
